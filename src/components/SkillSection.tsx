...
const SkillSection = () => {
  ...
  const filtered = useMemo(
    () => (activeGroup === 'All' ? skills : skills.filter(skill => skill.group === activeGroup)),
    [activeGroup]
  );
    ...
    >
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-muted mt-2">Filter by domain to see where I spend the most time.</p>
        </div>
        ...
        {filtered.map((skill, idx) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={levelBadge(skill.level)} style={levelBadgeStyle(skill.level)}>
                {skill.level}
              </span>
            </div>
            <div className="mt-4 space-y-2">
              <div
                ...
                aria-valuemax={100}
                aria-valuenow={
                  skill.level === 'Advanced' ? 100 : skill.level === 'Intermediate' ? 70 : 40
                }
              >
                <div
              ...
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                Recently used in:{' '}
                {skill.group === 'Frontend' ? 'React/React Native shipping' : skill.category}
              </p>
            </div>
          </article>
        ))}
