...
import { FaGithub, FaExternalLinkAlt, FaLightbulb, FaTools, FaUserTie } from 'react-icons/fa';
...
const highlightText = (text: string) => {
  ...
  return text.split(pattern).map((chunk, index) => {
    const match = KEYWORDS.find(keyword => keyword.toLowerCase() === chunk.toLowerCase());
    if (match) {
      return (
...
const ImageWithFallback: React.FC<{ src?: string; alt?: string; category?: string }> = ({
  ...
  return (
    <div className="relative aspect-video w-full overflow-hidden rounded-2xl bg-zinc-800">
      {!isLoaded && !hasError && <div className="absolute inset-0 animate-pulse bg-zinc-700" />}
      <img
...
const ThreeDCard: React.FC<ThreeDCardProps> = ({
  ...
  const caseStudy = useMemo(
    () => ({
      problem: problem || points[0] || 'Addressing specific user needs and technical challenges.',
      solution:
        solution ||
        points[1] ||
        description?.slice(0, 100) + '...' ||
        'Built a comprehensive solution.',
      role: role || 'Full-stack development, UI/UX implementation, and deployment.',
      impact:
        impact ||
        (rating
        ...
        >
          ...
          >
            ...
            <div className="flex-1 space-y-3 text-sm">
              {/* Quick Summary */}
              <div className="text-muted line-clamp-2">{points[0] && highlightText(points[0])}</div>
                    ...
                    <div className="space-y-3 pt-3 border-t border-white/10">
                      ...
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted text-sm">{caseStudy.problem}</p>
                        </div>
                      </div>

                      {/* Solution */}
                      <div className="flex gap-3">
                        <FaTools className="text-green-400 mt-1 flex-shrink-0" aria-hidden="true" />
                        <div>
                          <p className="text-xs text-green-400 font-semibold uppercase tracking-wide mb-1">
                            Solution
                          </p>
                          <p className="text-muted text-sm">{caseStudy.solution}</p>
                        </div>
                      </div>
